...
form.addEventListener("submit", function (event) {
    ...
    urlDisplay.innerHTML = `Your resume URL: <a href="${uniqueUrl}" target="_blank">${uniqueUrl}</a>`;
    // Check if the resume-preview element exists before appending
    const resumePreview = document.querySelector(".resume-preview");
    if (resumePreview) {
        resumePreview.appendChild(urlDisplay);
    }
    else {
        console.error("Element with class 'resume-preview' not found.");
    }
});
...
function generateUniqueUrl(username) {
    ...
    return `${formattedUsername}.vercel.app/resume`;
}
...
editButton === null || editButton === void 0 ? void 0 : editButton.addEventListener("click", enableEditing);
